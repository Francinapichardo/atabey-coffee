...
// Detecta el scroll para cerrar el menú y contraer/expandir el botón flotante según la posición
window.addEventListener('scroll', () => {
    ...
    // Si bajas más de 50px se contrae a solo el icono; si regresas arriba, se expande completo
    if (currentScroll > 50) {
...
function triggerChaos() {
    ...
    let randomIndex;
    do {
        randomIndex = Math.floor(Math.random() * chaosPicks.length);
    } while (randomIndex === lastChaosIndex);
    lastChaosIndex = randomIndex;
    
    const randomChaos = chaosPicks[randomIndex];
    textBox.style.opacity = 0;
    setTimeout(() => {
